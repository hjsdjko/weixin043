	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discusschongwuxinxi`, this.ruleForm.id);
...
//跨表计算判断
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`discusschongwuxinxi`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`discusschongwuxinxi`, this.ruleForm);
						}else{
							await this.$api.add(`discusschongwuxinxi`, this.ruleForm);
						}
						uni.setStorageSync('discusschongwuxinxiCleanType',true);
